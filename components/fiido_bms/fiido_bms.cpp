...
void FiidoBMSHub::parse_battery_(const uint8_t *p, size_t len) {
  ...
  ESP_LOGV(TAG, "[%s] BATTERY V=%.1f I=%.1f Ah=%.1f", this->parent_->address_str(), u16be(p, 4) / 10.0,
...
void FiidoBMSHub::parse_motor_(const uint8_t *p, size_t len) {
  ...
  ESP_LOGV(TAG, "[%s] MOTOR ver=%u T=%dC wheel=%.1f cap=%u", this->parent_->address_str(), p[0], temp_c,
...
void FiidoBMSHub::parse_energy_(const uint8_t *p, size_t len) {
  ...
  ESP_LOGV(TAG, "[%s] ENERGY torque=%.1fNm rpm=%u this=%.1fWh total=%.1fWh up=%us", this->parent_->address_str(),
           torque / 10.0, rpm, u16be(p, 4) / 10.0, u32be(p, 6) / 10.0, u16be(p, 10));
}
...
void FiidoBMSHub::parse_meter_(const uint8_t *p, size_t len) {
  ...
  ESP_LOGV(TAG, "[%s] METER HW=%u SW=%u modeData=%u", this->parent_->address_str(), p[0], p[1], p[7]);
}
...
void FiidoBMSHub::parse_speed_limit_(const uint8_t *p, size_t len) {
  ...
  ESP_LOGV(TAG, "[%s] SPEED_LIMIT value=%u 0x%02X", this->parent_->address_str(), p[0], p[0]);
  if (this->speed_limit_select_ != nullptr && this->addr_27_valid_) {
    bool limit_on = (this->addr_27_cache_ & 0x20) != 0;
    const char *opt = nullptr;
    if (p[0] == 100)
      opt = "No limit";
    else if (limit_on && p[0] == 6)
      opt = "6 km/h";
    else if (limit_on && p[0] == 25)
      opt = "25 km/h";
    if (opt != nullptr) {
      publish_(this->speed_limit_select_, opt);
    } else {
      ...
      if (this->last_ambiguous_limit_log_ms_ == 0 ||
        ...
        ESP_LOGW(TAG, "[%s] SPEED_LIMIT ambiguous (value=%u bit5=%u, %u since last log) keep prev",
                 this->parent_->address_str(), p[0], limit_on ? 1 : 0, (unsigned)this->ambiguous_limit_count_);
...
void FiidoBMSHub::write_masked_bits_(uint8_t addr, uint8_t mask, uint8_t bits, uint8_t *cache, const char *name) {
...
void FiidoBMSHub::write_flag_bit_(uint8_t addr, uint8_t mask, bool set, uint8_t *cache, const char *name) {
...
bool FiidoBMSHub::write_value_byte_(FrameType type, uint8_t addr, uint8_t value, const char *name) {
