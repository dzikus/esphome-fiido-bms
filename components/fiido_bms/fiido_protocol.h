...
[[nodiscard]] MaskedWrite compute_masked_write(uint8_t addr, uint8_t cache, uint8_t mask,
                                               uint8_t new_bits);
...
[[nodiscard]] size_t build_write_frame(FrameType type, uint8_t addr, const uint8_t *payload,
...
[[nodiscard]] bool validate_notify(const uint8_t *buf, size_t len, uint8_t *out_addr,
...
[[nodiscard]] BurstGate evaluate_burst_gate(uint32_t now, uint32_t interval, uint32_t phase,
...
[[nodiscard]] FlagView decode_flags(uint8_t b27, uint8_t b28, uint8_t b2B, uint8_t b2C,
                                    uint8_t b38, uint8_t b39);
